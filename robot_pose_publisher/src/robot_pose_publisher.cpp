...
int main(int argc, char **argv)
{
  ...
  bool is_stamped;
  ros::Publisher p_pub;
  ros::Publisher p_pub_no_stamped;
  ...
  if (is_stamped)
    p_pub = nh.advertise<geometry_msgs::PoseStamped>("mavros/vision_pose/pose", 1);
  else
    p_pub = nh.advertise<geometry_msgs::Pose>("mavros/vision_pose/pose", 1);

  p_pub_no_stamped = nh.advertise<geometry_msgs::Pose>("mavros/vision_pose/pose_no_stamped", 1);
  ...
  while (nh.ok())
  {
    ...
    try
    {
      ...
      p_pub_no_stamped.publish(pose_stamped.pose);
    }
